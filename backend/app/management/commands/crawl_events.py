...
class Command(BaseCommand):
    ...
    def _process_file_events(self, processor: EventProcessor, file_path: str):
        ...
        try:
            self.stdout.write(f"Loading event URLs from {file_path}")
            with open(file_path, "r") as f:
                urls = [line.strip() for line in f]
        except Exception as e:
            self.stderr.write(self.style.ERROR(f"Failed to load event URLs: {str(e)}"))
            return

        if not urls:
            self.stdout.write(self.style.WARNING("No event URLs found in file"))
            return
